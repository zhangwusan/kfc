...
class GradientDescentOptimizer(BaseOptimizer):
    """Finite-difference gradient descent for smooth scalar objectives."""

    def __init__(self, lr: float = 0.05, max_iter: int = 200, eps: float = 1e-5) -> None:
        self.lr = float(lr)
    ...
    def optimize(self, objective: Callable[[float], float], initial_value: float) -> float:
        ...
        for _ in range(self.max_iter):
            grad = (objective(x + self.eps) - objective(x - self.eps)) / (2.0 * self.eps)
            x = x - self.lr * grad
        return x
